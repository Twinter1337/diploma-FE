...
export default function TrainerCard({ trainer }: TrainerCardProps) {
  ...
  const avatarSize = isMobile ? 80 : undefined;

  const avatarBlock = (
    <div style={{ position: 'relative', flexShrink: 0 }}>
      <div style={{
        width: isMobile ? avatarSize : '100%',
        height: isMobile ? avatarSize : undefined,
        ...
        {!showGradient ? (
          ...
          <>
            ...
            <span style={{
              color: 'white', fontWeight: 700, fontSize: isMobile ? 28 : 44,
  ...
  return (
    ...
    >
      ...
      {/* Info */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 6 : 8, flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8 }}>
          <div style={{ minWidth: 0 }}>
            <h3 style={{
              margin: 0, fontSize: isMobile ? 15 : 16.5, fontWeight: 600, color: '#0F172A',
              letterSpacing: '-0.01em', lineHeight: 1.3,
            }}>{trainer.firstName} {trainer.lastName}</h3>
            {isMobile && isVerified && (
              <span style={{
                display: 'inline-flex', alignItems: 'center', gap: 4,
                fontSize: 11, fontWeight: 600, color: 'var(--accent-700)', marginTop: 3,
              }}>
          ...
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
            <span style={{ color: '#F5A524', display: 'inline-flex' }}><StarFilled size={13} /></span>
            <span style={{ fontSize: 13, fontWeight: 600, color: '#0F172A' }}>{trainer.rating.toFixed(1)}</span>
            <span style={{ fontSize: 12, color: '#6B7280' }}>({trainer.reviewsCount})</span>
          </div>
        </div>

        {trainer.specializationTags.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
            {trainer.specializationTags.slice(0, isMobile ? 2 : undefined).map(tag => (
        ...
        {trainer.minPrice != null && (
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginTop: isMobile ? 0 : 2 }}>
            <span style={{ fontSize: 12, color: '#6B7280' }}>від</span>
            <span style={{ fontSize: isMobile ? 16 : 18, fontWeight: 700, color: '#0F172A', letterSpacing: '-0.01em' }}>
              {trainer.minPrice}
            </span>
            <span style={{ fontSize: 13, color: '#6B7280' }}>грн</span>
          </div>
        )}
